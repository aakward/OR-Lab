...
#def compute_grad_loss_h(lmbda,x,y,model_w):
#    grad=[]
#    s=compute_loss_h(model_w,x,y)
#    for j in range(d):
#            g=(lmbda*model_w[j]/n)-((y*x[j])*(math.exp(s)-1)*(1.0/n)/math.exp(s))
#            grad.append(g)
#    return grad
...
plt.title("Objective function in different epochs for different lambdas for logistic loss")
plt.xlabel("Epochs")
