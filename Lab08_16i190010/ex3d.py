...
print(covertype.data.shape)
A = covertype.data
#check the shape of iris target
print(covertype.target.shape)
#How many labels does iris data have?
#C=num_of_classes
#print(C)
n = covertype.data.shape[0] #Number of data points
d = covertype.data.shape[1] #Dimension of data points
#In the following code, we create a nx1 vector of target labels
y = 1.0*np.ones([A.shape[0],1])
for i in range(covertype.target.shape[0]):
    if covertype.target[i]==1:    
...
for i in indexarr:
    if(c<=0.8*n):
        train_data.append(covertype.data[i])
        train_label.append(y[i])
        c=c+1
    else:
        test_data.append(covertype.data[i])
...
for i1 in range(len(lmbda_set)):
    w= OPT1(train_data,train_label,lmbda_set[i1],num_epochs)
    temp1=compute_accuracy(train_data,train_label,w)
    temp2=compute_accuracy(test_data,test_label,w)
    train_acc.append(temp1)
    test_acc.append(temp2)
for i1 in range(len(lmbda_set)):
    print "For lambda=",lmbda_set[i1]," train set accuracy is: ",train_acc[i1]," and test set accuracy is: ",test_acc[i1]
